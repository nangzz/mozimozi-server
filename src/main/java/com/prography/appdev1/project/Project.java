...
public class Project {
	...
	public @ResponseBody SignUpVo SignUpCheck(@RequestBody Map<String, Object> json) {
		...
		try {
			...
			signUp.setMessage("회원가입에 실패하였습니다. 이미 회원 정보가 있으니 아이디/비밀번호 찾기를 진행 해주세요.");
			e.printStackTrace();
		}
		return signUp;
	...
	public @ResponseBody IdCheckVo IdCheck(@RequestBody Map<String, Object> json) {
		...
		try {
			...
			if (idCheck.size() > 0) {
				id.setSuccess(false);
				id.setMessage("중복된 아이디입니다. 다른 아이디를 사용 해주세요.");
			} else {
				id.setSuccess(true);
			}
		...
		public @ResponseBody IdCheckVo IdSearch(@RequestBody Map<String, Object> json) {
			...
			try {
				...
				if (idCheck.size() > 0) {
					...
					id.setSuccess(false);
					id.setMessage("아이디가 없습니다. 회원가입을 해주세요.");
				}
	...
	public @ResponseBody LoginVo UserCheck(@RequestBody Map<String, Object> json) {
		...
		try {
			...
			if (loginresult.size() > 0) {
				...
				login.setMessage("로그인 실패하였습니다. 아이디 및 비밀번호를 확인 해주세요.");
			}
